...
const CropRotationPlanner = () => {
  ...
  const handleDownload = () => {
    ...
    logoImg.src = '/logo.png'; // Path relative to public folder
    // Draw logo (asynchronously)
    ...
    // If image fails to load, fallback to just content
    ...
    function drawContent() {
      ...
      doc.text('Crop Rotation Recommendation', leftMargin, y, { maxWidth: 180 });
      y += 10;
      doc.setFontSize(12);
      doc.setTextColor(100, 100, 100);
      doc.text('Generated by SeedSync', leftMargin, y);
      y += 14;
      // Section: Recommended Next Crop
      doc.setFontSize(15);
      doc.setTextColor(34, 197, 94);
      doc.setFont(undefined, 'bold');
      doc.text('Recommended Next Crop:', leftMargin, y);
      doc.setFont(undefined, 'normal');
      doc.setFontSize(13);
      doc.setTextColor(0, 0, 0);
      doc.text(recommendation.nextCrop, leftMargin + 65, y);
      y += 12;
      ...
      doc.setFontSize(12);
      doc.setTextColor(0, 0, 0);
      const justificationLines = doc.splitTextToSize(recommendation.justification, 170);
      doc.text(justificationLines, leftMargin, y);
      y += justificationLines.length * 7 + 8;
      // Section: Key Advantages
      doc.setFontSize(15);
      doc.setTextColor(34, 197, 94);
      doc.setFont(undefined, 'bold');
      doc.text('Key Advantages:', leftMargin, y);
      y += 8;
      doc.setFont(undefined, 'normal');
      doc.setFontSize(12);
      doc.setTextColor(0, 0, 0);
      recommendation.advantages.forEach((adv, i) => {
        doc.circle(leftMargin + 2, y + i * 8 - 2, 1.5, 'F');
        doc.text(adv, leftMargin + 8, y + i * 8);
      });
      y += recommendation.advantages.length * 8 + 10;
      // Section: 3-Season Plan Table
      if (recommendation.rotationPlan) {
        doc.setFontSize(15);
        doc.setTextColor(34, 197, 94);
        doc.setFont(undefined, 'bold');
        doc.text('Suggested 3-Season Plan:', leftMargin, y);
        y += 8;
        doc.setFont(undefined, 'normal');
        doc.setFontSize(12);
        doc.setTextColor(0, 0, 0);
        (doc as any).autoTable({
          startY: y,
          head: [['Season', 'Crop']],
          body: recommendation.rotationPlan.map((step, i) => [
            `Season ${i + 1}`,
            step
          ]),
          theme: 'grid',
