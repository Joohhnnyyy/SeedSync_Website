...
const CropRotationPlanner = () => {
  ...
  const handleDownload = () => {
    ...
    doc.text('Recommended Next Crop:', 14, y);
    doc.setFontSize(13);
    doc.setTextColor(0, 0, 0);
    doc.text(recommendation.nextCrop, 80, y);
    y += 10;
    ...
    doc.setFontSize(12);
    doc.setTextColor(0, 0, 0);
    const justificationLines = doc.splitTextToSize(recommendation.justification, 180);
    doc.text(justificationLines, 14, y);
    y += justificationLines.length * 7 + 4;

    doc.setFontSize(14);
    doc.setTextColor(34, 197, 94);
    doc.text('Key Advantages:', 14, y);
    y += 8;
    doc.setFontSize(12);
    doc.setTextColor(0, 0, 0);
    recommendation.advantages.forEach((adv, i) => {
      doc.circle(16, y + i * 7 - 2, 1.5, 'F');
      doc.text(adv, 20, y + i * 7);
    });
    y += recommendation.advantages.length * 7 + 6;

    if (recommendation.rotationPlan) {
      doc.setFontSize(14);
      doc.setTextColor(34, 197, 94);
      doc.text('Suggested 3-Season Plan:', 14, y);
      y += 8;
      doc.setFontSize(12);
      doc.setTextColor(0, 0, 0);
      // Use autoTable for the plan
