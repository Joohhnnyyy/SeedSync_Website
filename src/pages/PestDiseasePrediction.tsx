...
const PestDiseasePredictionNew = () => {
  ...
  const handleDownload = () => {
    ...
    function drawContent(y) {
      ...
      doc.text('Pest & Disease Prediction Report', 14, y);
      doc.setFontSize(12);
      doc.setTextColor(0, 0, 0);
      doc.text('Generated by SeedSync', 14, y + 8);
      y += 18;
      doc.setFontSize(14);
      doc.setTextColor(34, 197, 94); // emerald-600
      doc.text('Likely Pests or Diseases:', 14, y);
      y += 8;
      doc.setFontSize(12);
      doc.setTextColor(0, 0, 0);
      const pests = result.likelyPests.split('\n').filter(Boolean);
      pests.forEach((p, i) => {
        doc.circle(16, y + i * 7 - 2, 1.5, 'F');
        doc.text(p, 20, y + i * 7);
      });
      y += pests.length * 7 + 6;
      doc.setFontSize(14);
      doc.setTextColor(34, 197, 94);
      doc.text('Symptoms to Watch For:', 14, y);
      y += 8;
      doc.setFontSize(12);
      doc.setTextColor(0, 0, 0);
      ...
      doc.setFontSize(12);
      doc.setTextColor(0, 0, 0);
      const measures = result.preventiveMeasures.split('\n').filter(Boolean);
